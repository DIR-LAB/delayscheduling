...
from HPCSimSkip_backfil import *
...
def categorical_policy(x, a, mask, action_space, attn):
    ...
    if attn:
        ...
        output_layer = kernel_skip(x, act_dim)
    # output_layer_sigmoid = tf.nn.sigmoid(output_layer)
    # output_layer_sigmoid_concat = tf.concat([output_layer_sigmoid, 1-output_layer_sigmoid], 1)
    # logp_all = tf.log(tf.clip_by_value(output_layer_sigmoid_concat, 1e-15, 1.0))
    # pi = tf.squeeze(tf.multinomial(output_layer_sigmoid_concat, 1), axis=1)
...
class PPOBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma=0.99, lam=0.95):
        size = size * 1000  # assume the traj can be really long
        self.obs_buf = np.zeros(combined_shape(size, obs_dim), dtype=np.float32)
        # self.cobs_buf = np.zeros(combined_shape(size, JOB_SEQUENCE_SIZE*3), dtype=np.float32)
        self.cobs_buf = None
        self.act_buf = np.zeros(combined_shape(size, act_dim), dtype=np.float32)
        self.mask_buf = np.zeros(combined_shape(size, 2), dtype=np.float32)
...
def ppo(workload_file, model_path, ac_kwargs=dict(), seed=0,
        ...
        shuffle=False,
        backfil=False, skip=False, score_type=0, batch_job_slice=0, sched_algo=4):
    ...
    for epoch in range(epochs):
        ...
        while True:
            ...
            if d:
                t += 1
                buf.finish_path(r)
                logger.store(EpRet=ep_ret, EpLen=ep_len, ShowRet=show_ret, SJF=sjf, F1=f1, SkipRatio=skip_count/ep_len)
                [o, co], r, d, ep_ret, ep_len, show_ret, sjf, f1, skip_count = env.reset(), 0, False, 0, 0, 0, 0, 0, 0
                if t >= traj_per_epoch:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--workload', type=str,
                        default='./data/SDSC-SP2-1998-4.2-cln.swf')  # RICC-2010-2 lublin_256.swf SDSC-SP2-1998-4.2-cln.swf
    ...
    if args.pre_trained:
        ...
        ppo(workload_file, args.model, gamma=args.gamma, seed=args.seed, traj_per_epoch=args.trajs, epochs=args.epochs,
            ...
            batch_job_slice=args.batch_job_slice, sched_algo=args.sched_algo)
    else:
        ppo(workload_file, args.model, gamma=args.gamma, seed=args.seed, traj_per_epoch=args.trajs, epochs=args.epochs,
