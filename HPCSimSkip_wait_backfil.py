...
class HPCEnvSkip(gym.Env):
    ...
    def build_observation(self):
        ...
        delta_wait = SKIP_TIME*len(self.job_queue)
        normalized_delta_wait = min(delta_wait/(MAX_SKIP_TIME*JOB_SEQUENCE_SIZE), 1.0 - 1e-5)
        print(normalized_delta_wait)
        ...
        if not self.cluster.can_allocated(job):
            free_node = self.cluster.free_node
            num_procs_per_node = self.cluster.num_procs_per_node
            running_jobs_copy = self.running_jobs[:]
            job_queue = self.job_queue[:]
            def can_allocated(job, free_node):
                if job.request_number_of_nodes != -1 and job.request_number_of_nodes > free_node:
                    return False
                if job.request_number_of_nodes != -1 and job.request_number_of_nodes <= free_node:
                    return True

                request_node = int(math.ceil(float(job.request_number_of_processors)/float(num_procs_per_node)))
                job.request_number_of_nodes = request_node
                if request_node > free_node:
                    return False
                else:
                    return True
            earliest_start_time = self.current_timestamp
            # sort all running jobs by estimated finish time
            running_jobs_copy.sort(key=lambda running_job: (running_job.scheduled_time + running_job.request_time))
            free_processors = self.cluster.free_node * self.cluster.num_procs_per_node
            for running_job in running_jobs_copy:
                free_processors += len(running_job.allocated_machines) * self.cluster.num_procs_per_node
                earliest_start_time = (running_job.scheduled_time + running_job.request_time)
                if free_processors >= job.request_number_of_processors:
                    break
            cpu_second = 0
            if not can_allocated(job, free_node):

                # try to backfill as many jobs as possible. Use FCFS
                job_queue.sort(key=lambda _j: self.fcfs_score(_j))
                job_queue_iter_copy = list(job_queue)
                for _j in job_queue_iter_copy:
                    if (self.current_timestamp + _j.request_time) < earliest_start_time:
                        if can_allocated(_j, free_node):
                            # we should be OK to schedule the job now
                            assert _j.scheduled_time == -1  # this job should never be scheduled before.
                            request_node = int(math.ceil(float(_j.request_number_of_processors) / float(num_procs_per_node)))
                            cpu_second += request_node * _j.request_time
                            free_node -= request_node
                            running_jobs_copy.append(_j)
                            job_queue.remove(_j)
            total_cpu_second = (earliest_start_time - self.current_timestamp)*self.loads.max_nodes
            delta_util = (cpu_second+1e-5)/(total_cpu_second+1e-5)
            delta_util = min(delta_util/1e-9, 1)

        self.pairs.append([job, normalized_wait_time, normalized_run_time,
                           normalized_request_nodes,normalized_free_nodes, can_schedule_now, normalized_skip_time, normalized_delta_wait, delta_util])
        for i in range(0, MAX_QUEUE_SIZE):
            vector[i * JOB_FEATURES:(i + 1) * JOB_FEATURES] = self.pairs[i][1:]
